...
Remember to refer to the [*Geocomputation with R*](https://geocompr.robinlovelace.net/index.html) book for more help, as well as [terra's website](https://rspatial.org/terra/pkg/index.html), especially the [tutorials](https://rspatial.org/terra/spatial/index.html).
...
There are a few commonly-used sources for global bathymetry data, including ETOPO (<https://www.ngdc.noaa.gov/mgg/global/global.html>) and GEBCO (<https://www.gebco.net/>). You can manually download these data from the websites, but today we'll try and use the `marmap` package () to download data from inside RStudio.
...
I chose monthly ('1 mo') data for August 2022, downloaded this as 'GeoTIFF (raster)', at 0.1 degrees resolution.
...
I saved the file, called `AQUA_MODIS.20220801_20220831.L3m.MO.SST.sst.9km.nc` in a folder called `data` in my working folder. Remember, we don't want to touch our raw data and we want to keep it separate from things we are working on and manipulating (e.g, the derived raster `papoue_terra` that I wrote to a file called `test_output.tiff` in the `output` folder).

I've put this file on Blackboard if you want to download it from there instead. Remember to put it in a folder called `data` in your working directory.
...
sst <- rast("data/AQUA_MODIS.20220801_20220831.L3m.MO.SST.sst.9km.nc")
...
Immediately we got a warning that the raster had an unknown extent. When we print `sst` we see that the extent is indeed unexpected (try `ext(sst)`): 0, 1, 0, 1. We expect that to be in degrees (longitude and latitude). Also, we see that the coordinate reference system is missing (try `crs(sst)`). When we plot it it looks okay, but notice the values of the axes.

You could look at some tutorials on how to read these NetCDF files into a sensible format (for example [this one](https://rpubs.com/Shanto_khandoker/659894)), but let's again keep it simple and download a GeoTIFF from the site. I go back and click on the image dimensions next to the highest resolution download (0.1 degrees). Highlighted in red in the screenshot below:

![](images/NASA_sst_download_screenshot_02.png)

That allows us to save a file called `MYD28M_2022-08-01_rgb_3600x1800.tiff`, which I also put on Blackboard. This is a raster file; let's read it in.

```{r}
# Read in the SST raster
sst_raster <- rast("data/MYD28M_2022-08-01_rgb_3600x1800.tiff")

# Check it's properties
sst_raster # seems much better

# And plot it
plot(sst_raster, legend = TRUE) # looks better

# But...
# No SST values, because this is a GeoTIFF image!
# The value in each grid cell represents a colour instead of an SST value.
values(sst_raster)[1:10] # Look at the first ten values, for example
```

The website says:

> In addition to the standard file formats---JPEG, PNG, GeoTIFF, or GoogleEarth (KMZ)---many (not all) datasets support these "data value" formats. (**Note:** These data values are only approximations based upon the scaled range of the imagery. These values should not be used as scientific absolutes but only for simple analysis.

So, we should use the raw data in the NetCDF file, or find another source for data. This is why it's important to carefully explore our data before we start working with it. It's also an example of some of the frustrating steps we have to work through when finding data to use.
...
We'll discuss getting data (including working with NetCDF) in more detail in a later lab, for the moment, let's read in some SST files that I have downloaded and converted raster for this lab.
...
# grec doesn't yet accept terra rasters, so we first convert our raster to the old raster package raster with
sst_01_oldstyle <- raster(sst_01)
# Then we run the function
sst_fronts <- detectFronts(sst_01_oldstyle)
# And convert back to terra
sst_fronts <- rast(sst_fronts)
