...
library(CircStats) # required by adehabitatHR
...
ggplot(data = ngp,
       aes(x = distance)) +
         geom_histogram() +
  labs(title = "Step length distribution",
       x = "Step length (km)",
       y = "Frequency")
...
ggplot(data = ngp,
       aes(x = timestep)) +
         geom_histogram() +
  labs(title = "Step duration distribution",
...
ggplot(data = world) +
  ...
  # then, the tracking data
    geom_point(data = ngp_embc_data, aes(x = decimal_longitude,
                                y = decimal_latitude,
                                colour = as.factor(embc_label))) +
  # and then we 'zoom' the map -- note I use the limits we just calculated
    coord_sf(xlim = c(min_x, max_x),
            ylim = c(min_y, max_y))
```
...
# Notice that we use $layer to access the first layer of the raster
# to get the depths, here (unhelpfully!) named 'layer'
# Remember you can see the names of raster layers with:
# names(bathy)
